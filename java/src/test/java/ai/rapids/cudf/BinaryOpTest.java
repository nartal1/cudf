...
public class BinaryOpTest {
    ...
    public void testAdd() {
        ...
        try (ColumnVector icv1 = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector add = fcv1.add(fcv2);
                 ColumnVector expected = ColumnVector.fromBoxedFloats(11f, 30f, 130f, 45.3f, 100f, 160f, null)) {
            ...
            try (ColumnVector add = lcv1.add(lcv2);
                 ColumnVector expected = ColumnVector.fromBoxedLongs(11L, 22L, 33L, 44L, 55L, null, 200L)) {
                assertColumnsAreEqual(expected, add, "int64");
            }

            try (ColumnVector add = lcv1.add(bcv1);
                 ColumnVector expected = ColumnVector.fromBoxedLongs(0L, 9L, 126L, null, 55L, null, 200L)) {
                assertColumnsAreEqual(expected, add, "int64 + byte");
            }

            try (ColumnVector add = dcv1.add(dcv2);
                 ColumnVector expected = ColumnVector.fromBoxedDoubles(11.0, 30.0, 130.0, 45.3, 100.0, 160.0, null)) {
            ...
            try (ColumnVector add = lcv1.add(Scalar.fromFloat(1.1f));
                 ColumnVector expected = ColumnVector.fromBoxedFloats(2.1f, 3.1f, 4.1f, 5.1f, 6.1f,  null, 101.1f)) {
                assertColumnsAreEqual(expected, add, "int64 + scalar float");
            }

            try (ColumnVector add = Scalar.fromShort((short)100).add(bcv1);
                 ColumnVector expected = ColumnVector.fromBoxedShorts((short)99,(short)107,(short)223, null,(short)150,
                         (short)160,(short)200)) {
    ...
    public void testSub() {
        ...
        try (ColumnVector icv1 = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector sub = fcv1.sub(fcv2);
                 ColumnVector expected = ColumnVector.fromBoxedFloats(-9f, -10f, 70f, -34.7f, 0f, 40f, null)) {
                assertColumnsAreEqual(expected, sub, "float32");
            }

            try (ColumnVector sub = icv1.sub(fcv2, DType.FLOAT32);
                 ColumnVector expected = ColumnVector.fromBoxedFloats(-9f, -18f, -27f, -36f, -45f, null, 0f)) {
                assertColumnsAreEqual(expected, sub, "int - float");
            }

            try (ColumnVector sub = lcv1.sub(lcv2);
                 ColumnVector expected = ColumnVector.fromBoxedLongs(-9L, -18L, -27L, -36L, -45L, null, 0L)) {
                assertColumnsAreEqual(expected, sub, "int64");
            }

            try (ColumnVector sub = lcv1.sub(bcv1);
                 ColumnVector expected = ColumnVector.fromBoxedLongs(2L, -5L, -120L, null, -45L, null, 0L)) {
                assertColumnsAreEqual(expected, sub, "int64 - byte");
            }

            try (ColumnVector sub = dcv1.sub(dcv2);
                 ColumnVector expected = ColumnVector.fromBoxedDoubles(-9.0, -10.0, 70.0, -34.7, 0.0, 40.0, null)) {
                assertColumnsAreEqual(expected, sub, "float64");
            }

            try (ColumnVector sub = dcv2.sub(icv1);
                 ColumnVector expected = ColumnVector.fromBoxedDoubles(9.0, 18.0, 27.0, 36.0, 45.0, null, 0.0)) {
                assertColumnsAreEqual(expected, sub, "double - int");
            }

            try (ColumnVector sub = lcv1.sub(Scalar.fromFloat(1.1f));
                 ColumnVector expected = ColumnVector.fromBoxedFloats(-0.1f, 0.9f, 1.9f, 2.9f, 3.9f,  null, 98.9f)) {
                assertColumnsAreEqual(expected, sub, "int64 - scalar float");
            }

            try (ColumnVector sub = Scalar.fromShort((short)100).sub(bcv1);
                 ColumnVector expected = ColumnVector.fromBoxedShorts((short)101, (short)93, (short)-23, null,
    ...
    // The rest of the tests are very basic to ensure that operations plumbing is in place, not to exhaustively test
    ...
    public void testMul() {
        ...
        try (ColumnVector icv = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv.mul(Scalar.fromFloat(1.1f));
                 ColumnVector expected = ColumnVector.fromBoxedFloats(1 * 1.1f, 2 * 1.1f, 3 * 1.1f, 4 * 1.1f, 5 * 1.1f,  null, 100 * 1.1f)) {
                assertColumnsAreEqual(expected, answer, "int64 * scalar float");
            }

            try (ColumnVector answer = Scalar.fromShort((short)100).mul(icv);
                 ColumnVector expected = ColumnVector.fromBoxedInts(100 * 1, 100 * 2, 100 * 3, 100 * 4, 100 * 5, null, 100 * 100)) {
    ...
    public void testFloorDiv() {
        ...
        try (ColumnVector icv = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv.floorDiv(Scalar.fromFloat(1.1f));
                 ColumnVector expected = ColumnVector.fromBoxedFloats((float)Math.floor(1 / 1.1f), (float)Math.floor(2 / 1.1f),
                         (float)Math.floor(3 / 1.1f), (float)Math.floor(4 / 1.1f), (float)Math.floor(5 / 1.1f),  null,
    ...
    public void testPow() {
        ...
        try (ColumnVector icv = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv.pow(Scalar.fromFloat(1.1f));
                 ColumnVector expected = ColumnVector.fromBoxedFloats((float)Math.pow(1, 1.1f), (float)Math.pow(2, 1.1f),
                         (float)Math.pow(3, 1.1f), (float)Math.pow(4, 1.1f), (float)Math.pow(5, 1.1f),  null,
            ...
            try (ColumnVector answer = Scalar.fromShort((short)100).pow(icv);
                 ColumnVector expected = ColumnVector.fromBoxedInts((int)Math.pow(100, 1), (int)Math.pow(100, 2),
    ...
    public void testEqual() {
        ...
        try (ColumnVector icv = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv.equal(Scalar.fromFloat(1.0f));
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(1 == 1.0f, 2 == 1.0f, 3 == 1.0f, 4 == 1.0f, 5 == 1.0f,  null, 100 == 1.0f)) {
                assertColumnsAreEqual(expected, answer, "int64 == scalar float");
            }

            try (ColumnVector answer = Scalar.fromShort((short)100).equal(icv);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(100 == 1, 100 == 2, 100 == 3, 100 == 4, 100 == 5, null, 100 == 100)) {
                assertColumnsAreEqual(expected, answer, "scalar short == int32");
            }
        }
    }
    ...
    public void testNotEqual() {
        ...
        try (ColumnVector icv = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv.notEqual(Scalar.fromFloat(1.0f));
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(1 != 1.0f, 2 != 1.0f, 3 != 1.0f, 4 != 1.0f,
                         5 != 1.0f,  null, 100 != 1.0f)) {
                assertColumnsAreEqual(expected, answer, "int64 != scalar float");
            }

            try (ColumnVector answer = Scalar.fromShort((short)100).notEqual(icv);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(100 != 1, 100 != 2, 100 != 3, 100 != 4,
                         100 != 5, null, 100 != 100)) {
                assertColumnsAreEqual(expected, answer, "scalar short != int32");
            }
        }
    ...
    @Test
    public void testStringCategoryLessScalar() {
        assumeTrue(Cuda.isEnvCompatibleForTesting());
        try (ColumnVector a = ColumnVector.categoryFromStrings("a", "b", "c", "d");
             ColumnVector b = ColumnVector.categoryFromStrings("a", "b", "b", "a");
             ColumnVector c = ColumnVector.categoryFromStrings("a", null, "b", null)) {
            ...
            try (ColumnVector answer = b.less(s);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(true, false, false, true)) {
                assertColumnsAreEqual(expected, answer);
            }

            try (ColumnVector answer = c.less(s);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(true, null, false, null)) {
                assertColumnsAreEqual(expected, answer);
            }
        }
    ...
    @Test
    public void testStringCategoryGreaterScalar() {
        assumeTrue(Cuda.isEnvCompatibleForTesting());
        try (ColumnVector a = ColumnVector.categoryFromStrings("a", "b", "c", "d");
             ColumnVector b = ColumnVector.categoryFromStrings("a", "b", "b", "a");
             ColumnVector c = ColumnVector.categoryFromStrings("a", null, "b", null)) {
    ...
    @Test
    public void testStringCategoryLessOrEqualScalar() {
        assumeTrue(Cuda.isEnvCompatibleForTesting());
        try (ColumnVector a = ColumnVector.categoryFromStrings("a", "b", "c", "d");
             ColumnVector b = ColumnVector.categoryFromStrings("a", "b", "b", "a");
             ColumnVector c = ColumnVector.categoryFromStrings("a", null, "b", null)) {
            ...
            try (ColumnVector answer = b.lessOrEqual(s);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(true, true, true, true)) {
                assertColumnsAreEqual(expected, answer);
            }

            try (ColumnVector answer = c.lessOrEqual(s);
    ...
    @Test
    public void testStringCategoryGreaterOrEqualScalar() {
        assumeTrue(Cuda.isEnvCompatibleForTesting());
        try (ColumnVector a = ColumnVector.categoryFromStrings("a", "b", "c", "d");
             ColumnVector b = ColumnVector.categoryFromStrings("a", "b", "b", "a");
             ColumnVector c = ColumnVector.categoryFromStrings("a", null, "b", null)) {
            ...
            try (ColumnVector answer = b.greaterOrEqual(s);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(false, true, true, false)) {
                assertColumnsAreEqual(expected, answer);
            }

            try (ColumnVector answer = c.greaterOrEqual(s);
                 ColumnVector expected = ColumnVector.fromBoxedBooleans(false, null, true, null)) {
                assertColumnsAreEqual(expected, answer);
            }
        }
    ...
    public void testBitAnd() {
        ...
        try (ColumnVector icv1 = ColumnVector.fromBoxedInts(INTS_1);
            ...
            try (ColumnVector answer = icv1.bitAnd(Scalar.fromInt(0x01));
                 ColumnVector expected = ColumnVector.fromBoxedInts(1 & 0x01, 2 & 0x01, 3 & 0x01, 4 & 0x01, 5 & 0x01,  null, 100 & 0x01)) {
                assertColumnsAreEqual(expected, answer, "int32 & scalar int32");
            }

            try (ColumnVector answer = Scalar.fromShort((short)100).bitAnd(icv1);
                 ColumnVector expected = ColumnVector.fromBoxedInts(1 & 100, 2 & 100, 3 & 100, 4 & 100, 5 & 100, null, 100 & 100)) {
                assertColumnsAreEqual(expected, answer, "scalar short & int32");
            }
        }
    }
