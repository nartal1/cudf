...
final class BitVectorHelper {
    ...
    static void append(HostMemoryBuffer src, HostMemoryBuffer dst, long dstOffset, long rows) {
        assert dst.length * 8 - dstOffset >= rows : "validity vector bigger then available space on dst: " + (dst.length * 8 - dstOffset) + " copying space needed: " + rows;
    ...
    private static void shiftSrcLeftAndWriteToDst(HostMemoryBuffer src, HostMemoryBuffer dst, long dstOffset, int shiftByBits, long length) {
        ...
        int srcLeftMask = dstMask << (8 - shiftByBits) ; // the mask to save the left side of the bits before we shift
        ...
        if (((length % 8) + shiftByBits > 8) || length % 8 == 0) {
            /*
                Only if the last byte has data that has been shifted to spill over to the next byte execute the
                following statement.
             */
